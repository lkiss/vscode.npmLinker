...
export async function linkPackage(args: any) {
    ...
    const deleteLinkTask = providedTasks!.find(task => task.name === TaskName.OBG_DELETE_LINK);
    const createLinkTask = providedTasks!.find(task => task.name === TaskName.OBG_CREATE_LINK);
    const deleteNodeModulesTask = providedTasks!.find(task => task.name === TaskName.OBG_DELETE_NODE_MODULES);
    const linkTask = providedTasks!.find(task => task.name === TaskName.OBG_LINK);
